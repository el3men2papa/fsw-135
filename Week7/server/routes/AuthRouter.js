...
if(!user){
    res.status(403)
    return next(new Error("Username or Password are in correct no user"))
    }

    user.checkPassword(req.body.password, (err, isMatch) => {
    if(err){
        res.status(403)
        return next(new Error("Username or Password are incorrect error"))
    }
    if(!isMatch){
        res.status(403)
        return next(new Error("User or password are incorrect not match"))
    }
